...
## ⚠️ CRITICAL: The `--no-sync` Flag

**The root cause of all file lock issues**: `uv run` rebuilds packages by default!

```bash
# ❌ CAUSES FILE LOCKS - UV tries to rebuild paracle.exe
uv run python script.py

# ✅ NO FILE LOCKS - Reuses existing environment
uv run --no-sync python script.py
```

**Always use `--no-sync` when**:
- Testing code
- Running diagnostics
- Executing one-off commands
- When you don't need dependency updates

**Only omit `--no-sync` when**:
- You explicitly want to update dependencies
