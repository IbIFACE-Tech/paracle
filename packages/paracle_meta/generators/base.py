...
class GenerationResult(BaseModel):
    ...
    quality_score: float = Field(default=0.0, ge=0, le=10, description="Quality score 0-10")
...
"""
    ...
    def _calculate_cost(
        ...
        provider_costs = COSTS.get(provider, {"default": {"input": 0.002, "output": 0.008}})
    ...
    def _format_best_practices(self, practices: list[Any] | None) -> str:
        ...
        for p in practices:
            if hasattr(p, "title") and hasattr(p, "recommendation"):
                ...
                lines.append(f"- **{p.get('title', 'Practice')}**: {p.get('recommendation', '')}")
