...
@click.option("--approval/--no-approval", default=True, help="Require approval before commit")
@click.option("--conventional/--simple", default=True, help="Use conventional commit format")
@click.option("--sign/--no-sign", default=False, help="Sign commits with GPG")
@click.option("--prefix/--no-prefix", default=True, help="Prefix commits with agent name")
...
def status(repo_path: str):
    ...
    for change in changes:
        ...
        table.add_row(
            change.file_path,
            f"[{status_color}]{change.change_type}[/]"
        )
...
@click.option("--type", "-t", type=click.Choice([t.value for t in CommitType]), default="feat", help="Commit type")
...
def merge(branch_name: str, target: str, repo: str):
    ...
    try:
        ...
        console.print(
            f"[green]✓[/green] Merged '{branch_name}' into '{target}'"
        )
...
def pr_create(execution_id: str, title: str, body: str, repo: str):
    ...
    branch = next(
        (b for b in exec_branches if b.execution_id == execution_id),
        None
    )

    if not branch:
        console.print(
            f"[red]No branch found for execution '{execution_id}'[/red]"
        )
        return
...
def cleanup(target: str, repo: str):
    ...
    console.print(
        f"[green]✓[/green] Cleaned up {count} merged branches"
    )
