    ...
    try:
        ...
        console.print(
            Panel(
                ...
                f"Commit created: ✓\n" +
                (f"Pushed to remote: ✓" if push else ""),
                border_style="green",
            )
        )
...
async def _status_async():
    ...
    try:
        ...
        table.add_row("Modified", str(len(modified)), ", ".join(modified[:3]) + ("..." if len(modified) > 3 else ""))
        table.add_row("Added", str(len(added)), ", ".join(added[:3]) + ("..." if len(added) > 3 else ""))
        table.add_row("Deleted", str(len(deleted)), ", ".join(deleted[:3]) + ("..." if len(deleted) > 3 else ""))
        table.add_row("Untracked", str(len(untracked)), ", ".join(untracked[:3]) + ("..." if len(untracked) > 3 else ""))
        table.add_row("[bold]TOTAL[/bold]", f"[bold]{output.get('total_changes', 0)}[/bold]", "")
